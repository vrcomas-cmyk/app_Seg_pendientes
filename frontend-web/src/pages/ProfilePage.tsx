...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (hash.includes('type=recovery')) {
      setIsRecovery(true)
      toast('Escribe tu nueva contraseña', { icon: '🔑' })
    }
  }, [])
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        {isRecovery && (
          <p className="text-sm text-teal-600 bg-teal-50 rounded-lg px-4 py-2 mb-4">
        ...
        <div className="space-y-4">
          ...
          <button onClick={handleChange} disabled={loading}
            ...
            {loading ? 'Actualizando...' : isRecovery ? 'Restablecer contraseña' : 'Cambiar contraseña'}
          </button>
        </div>
      </div>
    </div>
