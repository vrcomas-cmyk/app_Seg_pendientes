...
export default function CrmVentasPage() {
  ...
  const saveUpdate = async () => {
    ...
    const { data: updated } = await supabase.from('crm_offer_items').select('*, crm_offers(id, tipo, estatus, crm_clients(id, solicitante))').eq('id', activeItem.id).single()
    if (updated) {
                      ...
                      return (
                        <tr key={item.id}
                          ...
                          <td className="px-3 py-2 whitespace-nowrap">
                            <Link
                              to={`/crm/${item.crm_offers?.crm_clients?.id}`}
          ...
          <div className="w-80 flex-shrink-0 bg-white rounded-xl border border-gray-200 flex flex-col"
            ...
            <div className="px-4 py-3 border-b border-gray-200 bg-gray-50 sticky top-0">
              <div className="flex justify-between items-start">
                ...
                <button onClick={() => setActiveItem(null)} className="text-gray-400 hover:text-gray-600 text-lg ml-2">×</button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs">
              ...
              <div className="border-t border-gray-100 pt-3 space-y-2">
                <Link
                  to={`/crm/${activeItem.crm_offers?.crm_clients?.id}/offer/${activeItem.offer_id}`}
                  className="block text-xs text-teal-600 hover:underline">
                  Ver oferta original →
                </Link>
                <Link
                  to={`/crm/${activeItem.crm_offers?.crm_clients?.id}`}
                  className="block text-xs text-teal-600 hover:underline">
