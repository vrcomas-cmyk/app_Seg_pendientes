...
<script>window.onload = () => window.print()</script>
  ...
  // Modal conversión UM — se renderiza en el JSX
  const renderConvModal = () => {
    if (!convModal) return null
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 z-[100] flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-2xl w-full max-w-sm">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-base font-bold text-gray-800">Conversión de UM requerida</h2>
            <p className="text-xs text-gray-400 mt-1">No hay conversión registrada para este material</p>
          </div>
          <div className="p-6 space-y-4">
            <div className="bg-gray-50 rounded-lg p-3 text-sm">
              <p className="font-mono font-semibold text-gray-800">{convModal.codigo}</p>
              <p className="text-gray-500 text-xs mt-1">Stock en: <strong>{convModal.umStock}</strong> · Salida en: <strong>{convModal.umSalida}</strong></p>
            </div>
            <div>
              <label className="text-xs text-gray-500 block mb-2">
                1 <strong>{convModal.umStock}</strong> = ¿cuántos <strong>{convModal.umSalida}</strong>?
              </label>
              <input type="number" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-teal-400"
                placeholder="Ej: 12" value={convFactor} onChange={e => setConvFactor(e.target.value)} autoFocus />
              {convFactor && parseFloat(convFactor) > 0 && (
                <p className="text-xs text-teal-600 mt-2 font-medium">
                  1 {convModal.umStock} = {convFactor} {convModal.umSalida}
                </p>
              )}
            </div>
            <div className="bg-teal-50 border border-teal-200 rounded-lg p-3 text-xs text-teal-700">
              La conversión se guardará en el catálogo para uso futuro
            </div>
            <div className="flex justify-between pt-1">
              <button onClick={() => { setConvModal(null); setSavingSalida(false) }}
                className="border border-gray-200 text-gray-600 px-4 py-2 rounded-lg text-sm">Cancelar</button>
              <button onClick={() => { const f = parseFloat(convFactor); if (f > 0) convModal.onConfirm(f) }}
